...
describe('Link', () => {
  ...
  it('uses the decoratedText prop as href', () => {
    const { getByRole } = render(
      <Link decoratedText="https://square.net">Link</Link>
    );
  ...
  it('applies http prefix when none is supplied', () => {
    const { getByRole } = render(
      <Link decoratedText="square.net">Link</Link>
    );
    expect(getByRole('link')).toHaveAttribute(
      'href',
      'http://square.net'
  ...
  it('does not apply a prefix when one is already supplied', () => {
    const { getByRole } = render(
      <Link decoratedText="ftp://square.net">Link</Link>
    );
    expect(getByRole('link')).toHaveAttribute(
      'href',
      'ftp://square.net'
