...
export default function(entity, options) {
  ...
  if (expandedUrl.indexOf(displayUrlSansEllipses) != -1) {
    ...
    // This gets shortened to "hi http://sq.co/xyzabc", with display_url = "…nname.com/foo"
    ...
    return stringSupplant(
      "<span class='sqco-ellipsis'>#{precedingEllipsis}<span #{invisible}>&nbsp;</span></span><span #{invisible}>#{beforeDisplayUrl}</span><span class='js-display-url'>#{displayUrlSansEllipses}</span><span #{invisible}>#{afterDisplayUrl}</span><span class='sqco-ellipsis'><span #{invisible}>&nbsp;</span>#{followingEllipsis}</span>",
      v
    );
  }
  return displayUrl;
